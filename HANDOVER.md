...
**Проблема с локальным polling (выявлена, НЕ РЕШЕНА):**
- `telegram-hub/duplex_poll.ps1` — написан, но PowerShell 5.1 ломает UTF-8 при чтении JSON с сервера
- `ConvertFrom-Json` / `WebClient` / `WebRequest` — все дают битый русский текст в файле
- Причина: PS5.1 на Windows не дружит с UTF-8 из коробки
- **Решение:** написан `telegram-hub/duplex_poll.py` (Python) — он работает с UTF-8 корректно
- **Новый AI должен:** при переключении дуплекса использовать Python-скрипт, НЕ PowerShell

### Изменённые файлы в этой сессии
- `telegram-hub/duplex_poll.ps1` — polling-скрипт (PS5.1, заведомо сломан для русского языка)
- `telegram-hub/duplex_poll.py` — polling-скрипт (Python, с UTF-8)
- `AGENTS.md` — обновлён раздел 12 (Duplex), добавлен раздел 1 (запрет деплоя)
- `HANDOVER.md` — добавлена эта секция

## Что было сделано в этой сессии (18.05.2026, 14:41) — cloud-restore

### Восстановление с архива other-pc
- Выполнен протокол `/start`: sync → чтение контекста → проверка архива
...
- **Добавлены файлы** (с other-pc):
  ...
  - `test_mcp_call.py` — тест MCP вызовов
